	...
	<div class="col-sm-7">
		<div class="panel panel-info">
			...
			<div class="panel-body table-responsive">
				...
				<?php echo form_open(base_url(). 'system_setting/system_settings/do_update', array('class' => 'form-horizontal form-groups-bordered', 'enctype'=> 'multipart/form-data'));?>
				...
				<select name="language" class="form-control"> <!--options for language-->
				<?php
					$getLanguage = $this->db->list_fields('language');
					foreach ($getLanguage as $field) {
						if ($field == 'phrase_id' || $field == 'phrase') //from database table language
						
						continue;
						
						$default_language = $this->db->get_where('settings', array('type' => 'language'))->row()->description; //insert in db in table settings
				...
				<div class="form-group">
					...
					<div class="col-sm-12">
						<input type="text" class="form-control" name="zoom_api_key" value="<?php echo $this->db->get_where('settings', array('type' => 'zoom_api_key'))->row()->description;?>">
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-12">
						<input type="text" class="form-control" name="zoom_api_secret_key" value="<?php echo $this->db->get_where('settings', array('type' => 'zoom_api_secret_key'))->row()->description;?>">
					</div>
				</div>
		...
		<div class="panel panel-info">
			...
			<div class="panel-wrapper collapse in" aria-expanded="true">
				<div class="panel-body table-responsive">
					...
					<?php echo form_open(base_url() . 'system_setting/system_settings/upload_logo', array('class' => 'form-horizontal form-groups-bordered validate', 'target' => '_top', 'enctype' => 'multipart/form-data'));?>		
					
					<div class="form-group"> 
						<label class="col-sm-12"><?php echo get_phrase('browse_image');?>*</label>        
						<div class="col-sm-12">
							<input type='file' class="form-control" name="userfile" onChange="readURL(this);" /required> <!-- readUrl to display what we choose for logo,  -->
							<img id="blah" src="<?php echo base_url();?>uploads/logo.png" alt="" height="200" width="200"/> <!-- blah(empty), readURL refer to java script from js file, from backend-->
						</div>
					</div>
					...
					<?php echo form_open(base_url() . 'system_setting/system_settings/themeSettings', array('class' => 'form-horizontal form-groups-bordered validate', 'target' => '_top', 'enctype' => 'multipart/form-data'));
					?>
					...
					<div class="radio radio-megna">
						<input type="radio" <?php if($skin = $this->db->get_where('settings' , array('type'=>'skin_colour'))->row()->description == 'megna') echo 'checked';?> name="skin_colour" id="radio5" value="megna">
					...
					<div class="form-group">
						<button type="submit" class="btn btn-block btn-info btn-rounded btn-sm">&nbsp;<?php echo get_phrase('change_theme');?></button>
					</div>
                    <?php echo form_close();?>
